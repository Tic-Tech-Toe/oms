...
import WhatsappConnectDialog from "./WhastappConnectDialog";
import ZohoConnectDialog from "./ZohoConnectDialog";
...
const WelcomePage = () => {
  ...
  const [showWhatsappConnectDialog, setShowWhatsappConnectDialog] =
    useState(false);
  const [showZohoConnectDialog, setShowZohoConnectDialog] = useState(false);
  ...
  useEffect(() => {
    ...
    setHasConsented(userDoc?.consent?.agreed || false);

    const subStatus = userDoc?.subscription?.status;
    const connections = userDoc?.connections || {};
    const allConnected =connections.whatsapp && connections.zoho;

      console.table({subStatus, allConnected, connections});

    // ✅ Only redirect to /orders if user is fully connected AND not in trial
    if (allConnected && subStatus && subStatus !== "trialing") {
  ...
  if (!userDoc) {
    return (
      <div className="py-6 bg-gray-200 text-center">Loading setup data...</div>
    );
  }

  // Determine Connection Status from userDoc
  const connections = userDoc.connections || {};
  const isWhatsappConnected = connections?.whatsapp?.connected;
  const isZohoConnected = connections?.zoho?.connected;
  // const isGoogleConnected = connections?.googleContacts?.connected;

  // Check if ALL connections are complete
  const isAllConnected =
    isWhatsappConnected && isZohoConnected
  ...
  const connectionSteps = [
    {
      ...
      description:
        "Connect your official WhatsApp number to send and receive messages.",
      connected: isWhatsappConnected,
      connectPath: "/api/setup/connect-whatsapp",
      buttonClr: "#199083",
      icon: "/connections/whatsapp-business-ico.svg",
      connectAction: () => setShowWhatsappConnectDialog(true),
    },
    {
      name: "Zoho CRM",
      ...
      connectAction: () => setShowZohoConnectDialog(true),
    },
    // {
    //   name: "Google Account (Drive & Contacts)",
    //   description:
    //     "Sync your Google Contacts and allow access to Drive for data/report storage.",
    //   connected: isGoogleConnected,
    //   connectPath: "/api/setup/connect-google",
    //   buttonClr: "#E02735",
    //   icon: "/connections/google.png",
    //   connectAction: () => setShowGoogleConnectDialog(true),
    // },
  ];
  ...
  const ConnectButtonWithLogo = ({ step, router, disabled }) => {
    ...
    return (
      ...
      >
        <div
          className={`p-1 rounded-full bg-white flex items-center justify-center ${logoSize}`}
        >
          <Image
            src={step.icon}
            alt={`${step.name} Logo`}
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="py-8 bg-white border-b border-gray-200 text-center">
        ...
        <p className="md:text-xl text-md mt-3 px-4  text-gray-500 font-light max-w-2xl mx-auto">
          Let's get your platform connected. Complete the quick setup below to
          begin.
        </p>
        <div></div>
      </div>
      ...
      <div className="max-w-3xl mx-auto p-6 space-y-10">
        ...
        >
          ...
          <ol className="relative space-y-8 mb-10">
            {connectionSteps.map((step, index) => (
              <li
                key={step.name}
                className={`flex items-start ${index < connectionSteps.length - 1 ? "border-l-2 border-dashed border-gray-200 ml-5 pl-5" : "ml-5 pl-5"}`}
              >
                {/* Timeline Circle */}
                {/* <span
                ...
                >
                  {step.connected ? (
                    ...
                    <div className="w-5 h-5 rounded-full bg-gray-400 text-white flex items-center justify-center font-bold text-xs">
                      {index + 1}
                    </div>
                  )}
                </span> */}
                ...
                <div className="flex justify-between items-center w-full">
                  <div className="pr-4">
                    <h3
                      className={`text-xl font-bold ${step.connected ? "text-gray-500" : "text-gray-900"}`}
                    >
                      {step.name}
                    </h3>
          ...
          <div className="mt-12 pt-6 border-t border-gray-200 text-center">
            {isAllConnected ? (
              <div className="space-y-4">
                <p
                  className={`text-lg font-medium flex items-center justify-center text-green-600`}
                >
                  <CheckCircle className="w-5 h-5 mr-2" /> All Setup Complete!
                </p>
                {userDoc?.subscription?.status === "trialing" ||
        ...
        />
        <ZohoConnectDialog
          open={showZohoConnectDialog}
          onOpenChange={setShowZohoConnectDialog}
        />
      </div>
    </div>
  );
