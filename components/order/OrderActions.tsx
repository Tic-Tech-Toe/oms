...
const OrderActions = ({row}: {row: Row<OrderType>}) => {
    const { setSelectedOrder, setOpenEditDialog, openEditDialog } = useOrderStore();
        ...
        <DropdownMenu>
            ...
            <DropdownMenuContent>
                {menuItems.map((item, i) => (
                    <DropdownMenuItem key={i} className={`flex items-center gap-2 p-[10px] ${item.className}`} onClick={() => handleClickedItem(item.label)}>
                        {item.icon}
